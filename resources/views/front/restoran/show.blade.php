...
'book_car' => $isEn ? 'Restoran Booking' : 'Booking Restoran',
'pickup_date' => $isEn ? 'Visit Date' : 'Tanggal Kunjungan',
'total_days' => $isEn ? 'Participants' : 'Peserta',
...
<section class="max-w-7xl mx-auto px-4 py-10">
  ...
  <div class="grid gap-10 lg:grid-cols-3">
    ...
    <div class="lg:col-span-2 space-y-8">
      ...
      <div class="rounded-3xl overflow-hidden shadow-sm border border-slate-200 bg-white">
        ...
        @else
        <img
          src="https://via.placeholder.com/1200x600?text=Restoran"
          alt="{{ $title }}"
    ...
    <aside class="lg:col-span-1">
      <div
        id="restoranBookingBox"
        data-price-per-pax="{{ (int) $package->price_per_pax }}"
  ...
  {{-- REVIEWS --}}
  <div class="mt-14">
    @include('front.partials.reviews', ['item' => $package, 'type' => 'restoran'])
  </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const box = document.getElementById('restoranBookingBox');
    const pricePerPax = box ? parseInt(box.dataset.pricePerPax || '0', 10) : 0;
    ...
    function recalc() {
      ...
      const pax = parseInt(participants.value, 10);
      daysEl.textContent = pax;
