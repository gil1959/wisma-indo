...
@section('title', $isEn ? 'Hotel/Vila' : 'Hotel/Vila')
        ...
        <div class="grid gap-8 lg:grid-cols-12 items-center">
            ...
            <div class="lg:col-span-7" data-aos="fade-up">
                <div class="inline-flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-extrabold"
                    ...
                    {{ $siteSettings['hotel_hero_badge'] ?? 'Hotel & Vila' }}

                </div>

                <h1 class="mt-4 text-3xl lg:text-4xl font-extrabold text-slate-900 leading-tight">
                    {{ $siteSettings['hotel_hero_title'] ?? 'Pilihan Hotel & Vila Terbaik untuk Anda' }}
                </h1>


                <p class="mt-3 max-w-2xl text-slate-600">
                    {{ $siteSettings['hotel_hero_desc'] ?? 'Akomodasi nyaman dengan fasilitas lengkap dan pelayanan terbaik.' }}
                </p>



                <div class="mt-6 flex flex-wrap gap-2">
                    <span class="pill pill-azure"><i data-lucide="shield-check" class="w-4 h-4"></i> {{ $siteSettings['hotel_chip1'] ?? 'Aman' }}</span>
                    <span class="pill pill-azure"><i data-lucide="wallet" class="w-4 h-4"></i> {{ $siteSettings['hotel_chip2'] ?? 'Nyaman' }}</span>
                    <span class="pill pill-azure"><i data-lucide="clock" class="w-4 h-4"></i> {{ $siteSettings['hotel_chip3'] ?? 'Bersih' }}</span>
                    <span class="pill pill-azure"><i data-lucide="map" class="w-4 h-4"></i> {{ $siteSettings['hotel_chip4'] ?? 'Strategis' }}</span>
                </div>
                ...
                <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-soft relative overflow-hidden">
                    ...
                    <div class="relative">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <div class="font-extrabold text-slate-900">{{ $siteSettings['hotel_note_title'] ?? 'Catatan' }}</div>
                                <div class="text-sm text-slate-600 mt-1">
                                    {{ $siteSettings['hotel_note_desc'] ?? 'Klik “Booking Sekarang” untuk lihat detail akomodasi.' }}
                                </div>
                        ...
                        <div class="mt-5 grid grid-cols-2 gap-3">
                            <div class="rounded-2xl border border-slate-200 bg-white p-4">
                                <div class="flex items-center gap-2 text-sm font-extrabold text-slate-900">
                                    ...
                                    {{ $siteSettings['hotel_note1_title'] ?? 'Nyaman' }}

                                </div>
                                <div class="text-xs text-slate-600 mt-1">{{ $siteSettings['hotel_note1_desc'] ?? 'Fasilitas lengkap' }}
                                </div>
                            </div>
                            ...
                            <div class="rounded-2xl border border-slate-200 bg-white p-4">
                                <div class="flex items-center gap-2 text-sm font-extrabold text-slate-900">
                                    ...
                                    {{ $siteSettings['hotel_note2_title'] ?? 'Bersih' }}

                                </div>
                                <div class="text-xs text-slate-600 mt-1">{{ $siteSettings['hotel_note2_desc'] ?? 'Kamar selalu terawat' }}
                                </div>
                            </div>
                            ...
                            <div class="rounded-2xl border border-slate-200 bg-white p-4">
                                <div class="flex items-center gap-2 text-sm font-extrabold text-slate-900">
                                    ...
                                    {{ $siteSettings['hotel_note3_title'] ?? 'Keluarga' }}

                                </div>
                                <div class="text-xs text-slate-600 mt-1">{{ $siteSettings['hotel_note3_desc'] ?? 'Cocok untuk liburan' }}
                                </div>
                            </div>
                            ...
                            <div class="rounded-2xl border border-slate-200 bg-white p-4">
                                <div class="flex items-center gap-2 text-sm font-extrabold text-slate-900">
                                    ...
                                    {{ $siteSettings['hotel_note4_title'] ?? 'Strategis' }}

                                </div>
                                <div class="text-xs text-slate-600 mt-1">{{ $siteSettings['hotel_note4_desc'] ?? 'Dekat tempat wisata' }}
                                </div>
                            </div>
                        </div>
                    </div>
...
<section class="max-w-7xl mx-auto px-4">
    <div class="card p-5 -mt-8 relative z-10" data-aos="fade-up" data-aos-delay="100">
        <form method="GET" action="{{ route('hotel.index') }}" class="grid gap-4 md:grid-cols-12 items-end">
            ...
            <div class="md:col-span-10">
                ...
                <div class="relative">
                    <input
                        ...
                        placeholder="{{ $isEn ? 'Example: Resort, Vila...' : 'Contoh: Resort, Vila...' }}"
            ...
            <div class="md:col-span-2 flex gap-3">
                ...
                <a class="btn btn-ghost w-full" href="{{ route('hotel.index') }}">
                    {{ $isEn ? 'Reset' : 'Reset' }}
                </a>
            </div>
        </form>
...
<section class="bg-slate-50">
    <div class="max-w-7xl mx-auto px-4 pb-14">
        <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <a href="{{ route('hotel.show', $package->slug) }}"
                ...
                <div class="px-4 pt-4 pb-3">
                    ...
                    <div class="mt-2 text-sm">
                        ...
                        <span class="font-extrabold text-rose-600">
                            Rp {{ number_format((int)$package->price_per_night, 0, ',', '.') }}
                        </span>
                        <span class="text-slate-500">{{ $isEn ? '/Night' : '/Malam' }}</span>
                    </div>
                </div>
            ...
            <div class="col-span-full">
                <div class="card p-10 text-center">
                    <div class="mx-auto h-14 w-14 rounded-2xl border flex items-center justify-center"
                        ...
                        <i data-lucide="building" class="w-6 h-6" style="color:#0194F3;"></i>
                    </div>
                    <h3 class="mt-4 text-lg font-extrabold text-slate-900">{{ $isEn ? 'No hotel packages yet' : 'Belum ada paket hotel' }}</h3>
