...
$destination = $isEn ? ($package->destination_en ?: $package->destination) : $package->destination;

$includes = $isEn ? ($package->includes_en ?: $package->includes) : $package->includes;
$excludes = $isEn ? ($package->excludes_en ?: $package->excludes) : $package->excludes;

$flightInfo = $package->flight_info;// ==== Gallery data for lightbox ====
$galleryImages = [];
...
$itineraryHeading = $isEn ? 'Trip Itinerary' : 'Itinerary Perjalanan';
$includeHeading = $isEn ? 'Included' : 'Termasuk (Include)';
$excludeHeading = $isEn ? 'Not Included' : 'Tidak Termasuk (Exclude)';
@endphp
...
<div
    x-data="{
        ...
        nextLb() {
            ...
            this.lbIndex = (this.lbIndex + 1) % this.gallery.length;
        },
        prevLb() {
            if (!this.gallery.length) return;
            this.lbIndex = (this.lbIndex - 1 + this.gallery.length) % this.gallery.length;
        }
    }"
    x-on:keydown.window.escape="if (lbOpen) closeLb()"
    x-on:keydown.window.arrow-right="if (lbOpen) nextLb()"
    x-on:keydown.window.arrow-left="if (lbOpen) prevLb()"
    ...
    <div class="md:col-span-2 space-y-8">
        ...
        <section class="bg-white rounded-xl shadow-sm p-5">
            <h2 class="text-lg font-semibold mb-3 text-[#0194F3]">
                {{ $isEn ? 'About This Package' : 'Tentang Paket' }}
            </h2>

            @php
            // Kalau data di DB ternyata double-escaped, ini bikin "&amp;" balik jadi "&".
            // Kalau nggak double-escaped, ini tetap aman.
            $descToRender = html_entity_decode($descHtml, ENT_QUOTES | ENT_HTML5, 'UTF-8');
            @endphp

            <div class="quill-content break-words">
                {!! $descToRender !!}
            </div>
        </section>
        @endif
...
{{-- PRO LIGHTBOX (Vanilla JS, no Alpine) --}}
...
<script>
    (function() {
        ...
        function open(i) {
            ...
            img.onerror = () => {
                // kalau gagal load, tetep tutup loading biar user nggak stuck
                mainImg.src = src;
                loading.classList.add('hidden');
                setCounter();
                setActiveThumb();
