        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1 space-y-3">
                ...
                <div class="relative group">
                    <div class="w-full aspect-[2/1] rounded-2xl border-2 border-dashed border-slate-300 overflow-hidden flex flex-col items-center justify-center bg-slate-50 hover:bg-slate-100 transition cursor-pointer relative" onclick="document.getElementById('seo_image_input').click()">
                        <img id="seo_image_preview" src="{{ $article->seo_image ? asset($article->seo_image) : '' }}" class="{{ $article->seo_image ? '' : 'hidden' }} w-full h-full object-cover absolute inset-0 z-0">
                        <div class="{{ $article->seo_image ? 'hidden' : 'flex' }} z-10 flex-col items-center justify-center text-slate-400 group-hover:text-[#0194F3]">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div>
                    ...
                    <input type="text" name="meta_keywords" id="meta_keywords" value="{{ old('meta_keywords', $article->meta_keywords) }}" class="w-full rounded-xl border-slate-300 focus:border-[#5438FF] focus:ring-[#5438FF]">
                </div>

                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Meta Description</label>
                    <textarea name="meta_desc" rows="3" class="w-full rounded-xl border-slate-300 focus:border-[#5438FF] focus:ring-[#5438FF]">{{ old('meta_desc', $article->meta_desc) }}</textarea>
                </div>

                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Social Title</label>
                    <input type="text" name="social_title" value="{{ old('social_title', $article->social_title) }}" class="w-full rounded-xl border-slate-300 focus:border-[#5438FF] focus:ring-[#5438FF]">
                </div>

                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Social Description</label>
                    <textarea name="social_desc" rows="3" class="w-full rounded-xl border-slate-300 focus:border-[#5438FF] focus:ring-[#5438FF]">{{ old('social_desc', $article->social_desc) }}</textarea>
                </div>

                <button type="submit" class="w-full bg-[#3B30FF] text-white px-4 py-3 rounded-xl font-bold hover:bg-blue-700 transition">Update Artikel</button>
            </div>
        </div>
    </form>
</div>
