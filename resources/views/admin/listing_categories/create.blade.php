...
@section('title', 'Tambah Kategori Iklan')
@section('page-title', 'Tambah Kategori Iklan')
...
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 max-w-2xl">
    <form action="{{ route('admin.listing-categories.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <select name="type" required class="w-full rounded-xl border-slate-300 focus:ring-[#0194F3] focus:border-[#0194F3]">
                ...
                <option value="property" {{ old('type') == 'property' ? 'selected' : '' }}>Properti (Contoh: Rumah, Gudang, Ruko)</option>
                <option value="goods" {{ old('type') == 'goods' ? 'selected' : '' }}>Barang (Contoh: Elektronik, Furniture)</option>
                <option value="services" {{ old('type') == 'services' ? 'selected' : '' }}>Jasa (Contoh: Renovasi, Pindahan)</option>
            </select>
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name') }}" required placeholder="Contoh: Gudang" class="w-full rounded-xl border-slate-300 focus:ring-[#0194F3] focus:border-[#0194F3]">
            @error('name') <span class="text-xs text-red-500 mt-1 block">{{ $message }}</span> @enderror
        </div>

        <div>
            <label class="block text-sm font-bold text-slate-700 mb-2">Foto/Gambar (Opsional tapi disarankan)</label>
            ...
            <div id="image-preview-container" class="mt-4 hidden">
                <p class="text-xs font-bold text-slate-500 mb-2">Preview Gambar:</p>
