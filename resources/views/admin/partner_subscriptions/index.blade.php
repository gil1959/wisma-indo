...
@section('title', 'Data Langganan Partner')
@section('page-title', 'Langganan Partner')
...
<div x-data="{ 
    ...
    <div class="px-5 py-4 border-b bg-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <div class="text-sm font-extrabold text-slate-900">Semua Permintaan Langganan Partner</div>
            <div class="text-xs text-slate-600 mt-1">Kelola transaksi pembelian paket bulanan oleh partner.</div>
        </div>
        
        <form action="{{ route('admin.partner_subscriptions.index') }}" method="GET" class="flex items-center gap-2">
        ...
        <table class="w-full text-left text-sm text-slate-600">
            <thead class="bg-slate-50 border-b text-xs font-extrabold text-slate-700">
                <tr>
                    ...
                    <th class="px-5 py-3">Paket Bulanan</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition">
                    ...
                    <td class="px-5 py-3">
                        <span class="font-bold text-slate-900">{{ $item->package->name }}</span> 
                        <span class="text-xs text-slate-500 uppercase block">{{ $item->package->duration_days }} Hari</span>
                    </td>
                    <td class="px-5 py-3 font-semibold text-emerald-600">Rp {{ number_format($item->amount, 0, ',', '.') }}</td>
                    <td class="px-5 py-3">
                        ...
                        @elseif($item->status == 'expired')
                            <span class="bg-slate-100 text-slate-700 px-3 py-1 rounded-full text-xs font-bold">Expired</span>
                        @else
                            <span class="bg-rose-100 text-rose-700 px-3 py-1 rounded-full text-xs font-bold">Rejected</span>
                        @endif
                    </td>
                    <td class="px-5 py-3 text-right">
                        ...
                        @if($item->status == 'pending')
                            <form action="{{ route('admin.partner_subscriptions.update', $item->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('PUT')
                        ...
                        <form action="{{ route('admin.partner_subscriptions.destroy', $item->id) }}" method="POST" class="inline-block">
                            @csrf
                            @method('DELETE')
                ...
                <tr>
                    <td colspan="7" class="px-5 py-10 text-center text-slate-500">
                        <div class="flex flex-col items-center justify-center">
                            <i data-lucide="package" class="w-10 h-10 text-slate-300 mb-2"></i>
                            <div>Belum ada transaksi langganan partner.</div>
                        </div>
                    </td>
                </tr>
                @endforelse
        ...
        <div x-show="rejectModal" style="display: none;" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            ...
            <div x-show="rejectModal" x-transition class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 z-10 text-left">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-xl font-bold text-slate-800">Tolak Langganan</h3>
                    <button type="button" @click="cancelReject()" class="text-slate-400 hover:text-slate-600"><i data-lucide="x" class="w-5 h-5"></i></button>
                </div>
                <p class="text-slate-500 mb-4 text-sm">Berikan alasan penolakan agar partner mengetahui kesalahannya.</p>
                ...
                <div class="flex gap-3 justify-end">
                    ...
                    <button type="button" @click="submitReject()" class="px-4 py-2 rounded-xl font-bold bg-orange-500 text-white hover:bg-orange-600 transition" :disabled="!rejectNote.trim()">Tolak Langganan</button>
                </div>
            </div>
        </div>
    </template>
    ...
    <template x-teleport="body">
        <div x-show="approveModal" style="display: none;" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            ...
            <div x-show="approveModal" x-transition class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 text-center z-10">
                ...
                <h3 class="text-xl font-bold text-slate-800 mb-2">Approve Langganan?</h3>
                <p class="text-slate-500 mb-6 text-sm">Paket akan diaktifkan dan kuota akan ditambahkan ke akun partner.</p>
