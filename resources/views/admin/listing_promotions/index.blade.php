...
@section('title', 'Data Promosi Iklan')
@section('page-title', 'Promosi Iklan')
...
<div x-data="{ 
    ...
    <div class="px-5 py-4 border-b bg-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <div class="text-sm font-extrabold text-slate-900">Semua Transaksi Promosi Iklan</div>
            <div class="text-xs text-slate-600 mt-1">Kelola transaksi pembelian fitur sundul atau premium.</div>
        </div>
        
        <form action="{{ route('admin.listing-promotions.index') }}" method="GET" class="flex items-center gap-2">
                ...
                <tr class="hover:bg-slate-50 transition">
                    ...
                    <td class="px-5 py-3 text-right">
                        ...
                        @if($item->status == 'pending')
                            <form action="{{ route('admin.listing-promotions.update', $item->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('PUT')
                        ...
                        <form action="{{ route('admin.listing-promotions.destroy', $item->id) }}" method="POST" class="inline-block">
                            @csrf
                            @method('DELETE')
                ...
                <tr>
                    <td colspan="7" class="px-5 py-10 text-center text-slate-500">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <div>Belum ada transaksi promosi iklan.</div>
                        </div>
                    </td>
                </tr>
                @endforelse
        ...
        <div x-show="rejectModal" style="display: none;" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            ...
            <div x-show="rejectModal" x-transition class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 z-10 text-left">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-xl font-bold text-slate-800">Tolak Transaksi</h3>
                ...
                <div class="flex gap-3 justify-end">
                    ...
                    <button type="button" @click="submitReject()" class="px-4 py-2 rounded-xl font-bold bg-orange-500 text-white hover:bg-orange-600 transition" :disabled="!rejectNote.trim()">Tolak Transaksi</button>
                </div>
            </div>
        </div>
    </template>
    ...
    <template x-teleport="body">
        <div x-show="approveModal" style="display: none;" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            ...
            <div x-show="approveModal" x-transition class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 text-center z-10">
                ...
                <h3 class="text-xl font-bold text-slate-800 mb-2">Approve Transaksi?</h3>
                <p class="text-slate-500 mb-6 text-sm">Visibilitas iklan akan ditingkatkan secara otomatis.</p>
