...
<div class="p-4 sm:p-6 lg:p-8">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div class="flex items-center justify-between p-6 border-b border-slate-100 bg-[#0194F3]">
            ...
            <div class="text-white/80 text-sm font-semibold italic">/terms-conditions</div>
        </div>

        <form action="{{ route('admin.legal.terms.update') }}" method="POST" class="p-6">
            @csrf
            
            <div class="mb-6">
                <label class="block text-sm font-bold text-slate-700 mb-2">Judul</label>
                <input type="text" name="title" value="{{ old('title', $title) }}" class="w-full rounded-xl border-slate-200 focus:border-[#1e293b] focus:ring focus:ring-[#1e293b] focus:ring-opacity-20" required>
                @error('title')
