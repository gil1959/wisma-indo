...
<div class="space-y-5">
    <div class="flex items-center justify-between">
        <h2 class="text-xl sm:text-2xl font-extrabold text-slate-900">Tambah Halaman Baru</h2>
    ...
    <form action="{{ route('admin.pages.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 space-y-6">
        @csrf
        
        <div class="space-y-2">
            <label class="text-sm font-bold text-slate-700">Judul Halaman</label>
            <input type="text" name="title" value="{{ old('title') }}" required placeholder="Contoh: Tentang Kami" class="w-full rounded-xl border-slate-200 focus:border-[#0194F3] focus:ring focus:ring-[#0194F3]/20 text-sm">
        ...
        <div class="space-y-2">
            ...
            <input type="text" name="slug" value="{{ old('slug') }}" required placeholder="Contoh: tentang-kami" class="w-full rounded-xl border-slate-200 focus:border-[#0194F3] focus:ring focus:ring-[#0194F3]/20 text-sm">
        ...
        <div class="space-y-2">
            ...
            <textarea name="content" id="editor" rows="10" placeholder="Tulis isi halaman di sini..." class="w-full rounded-xl border-slate-300 hidden">{{ old('content') }}</textarea>
            @error('content')<p class="text-xs text-rose-500 font-bold mt-1">{{ $message }}</p>@enderror
        </div>

        <div class="flex items-center gap-2">
            <input type="checkbox" name="is_active" value="1" id="is_active" {{ old('is_active', '1') == '1' ? 'checked' : '' }} class="rounded border-slate-300 text-[#0194F3] focus:ring-[#0194F3]">
