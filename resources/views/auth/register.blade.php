        ...
        <div class="space-y-6 mt-4">
            @if (!empty($siteSettings['google_login_active']) && $siteSettings['google_login_active'] == '1')
                <div class="text-center text-sm text-slate-600 mb-4">
                    {{ $isEn ? 'Please sign in using your Google account to continue.' : 'Silakan masuk menggunakan akun Google Anda untuk melanjutkan.' }}
                </div>
                ...
                <a href="{{ route('google.login') }}" class="w-full inline-flex justify-center items-center gap-2 rounded-xl px-4 py-3 bg-white border border-slate-200 text-slate-700 font-semibold shadow-sm hover:bg-slate-50 transition">
                    ...
                    {{ $isEn ? 'Sign in with Google' : 'Daftar dengan Google' }}
                </a>
            @else
                <div class="bg-red-50 text-red-500 text-sm p-4 rounded-xl text-center border border-red-100">
                    {{ $isEn ? 'Registration is currently disabled.' : 'Pendaftaran saat ini sedang dinonaktifkan.' }}
                </div>
            @endif

            <div class="text-center text-sm mt-6 pt-4 border-t border-slate-100">
