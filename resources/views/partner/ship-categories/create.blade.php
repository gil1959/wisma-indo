...
<div class="max-w-3xl mx-auto px-4 py-6">
    <div class="mb-5">
        <a href="{{ route('partner.ship-categories.index') }}"
           class="inline-flex items-center gap-2 text-sm font-extrabold text-slate-700 hover:text-slate-900">
            <i data-lucide="arrow-left" class="w-4 h-4"></i>
            Kembali
        </a>
        <h1 class="text-xl sm:text-2xl font-extrabold text-slate-900 mt-3">Tambah Kategori Sewa Kapal</h1>
        <p class="text-sm text-slate-600 mt-1">Buat kategori untuk paket sewa kapal.</p>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white shadow-sm p-5">
        <form action="{{ route('partner.ship-categories.store') }}" method="POST" class="space-y-4">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}"
                       ...
                       placeholder="Contoh: Kapal Luxury" required>
            </div>
